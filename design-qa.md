...
- Mobile app readiness: production build includes a web app manifest, app icons, iOS home-screen metadata, and a service worker that caches the shell, covers, and all 8 book manuscripts. Service worker cache is now `survivor-library-v7` so installed apps refresh this release.
...
- Reading guide layer: selected high-value sections across all 8 books now show a small "จับแก่น" card with core idea, memory hook, and self-check question. The layer is separate from manuscript files, can be turned off in Settings, and has a lightbulb shortcut in reader controls.
...
- Reading guide QA: Books 1-7 each render 5 guide cards and Book 8 renders 7 guide cards when enabled. Every book shows one lightbulb shortcut, and tapping it scrolls to a visible guide card. TOC still jumps to the correct source heading after guide insertion.
...
patches made after guide discoverability issue: added a lightbulb shortcut, clearer Settings copy, a guide pulse, and bumped the PWA cache to v6 so installed mobile apps update.
patches made after all-books expectation: expanded "จับแก่น" to all 8 books and changed the lightbulb shortcut to appear for any book with guide data; bumped the PWA cache to v7.
final result: passed
